...
keywords: [Spatiotemporal model, Integrated Nested Laplace Approximation, Marked log-Gaussian Cox process, SPDE approach, Wildfires, Climate projections]
citation:
...
if (!(file.exists("fire_data.zip"))){
  ...
  temp_file = tempfile()
  download.file("https://zenodo.org/record/7870593/files/fire_data.zip",temp_file,mode = 'wb' )
}
temp = unz("fire_data.zip", "DF_Aquitaine_sim.RData")
load(temp)
rm(temp)
unlink(temp_file)

#SAFRAN coordinates
temp = unz("fire_data.zip", "Coord_L2E_dep.csv")
...
Estimates of $\beta_1$ as defined in @eq-inla-proj are depicted in @tbl-slope, highlighting for which climate model there is a positive trend in wildfire activities (in red).  
